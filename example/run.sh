...
#   They cause Verilator to generate all simulation files and build them into
#   a single binary.
# - '--vpi' - flags enabling the Verilator features that the fault injection
#   relies on
# - config.vlt - file generated by faultergeist-gen, indicating to verilator
#   which signals should be exposed. Alternative is to pass '--public_flat_rw'.
...
$VERILATOR \
    --binary -j "$(nproc)" \
    --vpi \
    config.vlt worker/top.v worker/worker.v worker/comb_worker.v worker/dff_worker.v \
