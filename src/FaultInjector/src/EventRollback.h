...
class EventRollback {
   ...
   public:
    ...
    explicit EventRollback(const Signal* signal, uint64_t time, int bit_idx, s_vpi_value* nvalp)
        : id{getNextId()}, signal{signal}, time{time}, bit_idx{bit_idx}, vpi_value{*nvalp} {
    ...
    bool operator<(const EventRollback& other) const {
        if (time == other.time) {
            return id < other.id;
        }
    ...
    friend std::ostream& operator<<(std::ostream& os, const EventRollback& ev) {
        return os << "{[" << ev.time << "] id=" << ev.id << " " << ev.signal->path << "("
                  << ev.bit_idx << ")}";
    }
